...
@Singleton
public class CuratedCDCService {

    private final DataStorageService storage;
    private final String curatedTablePath;

    @Inject
    public CuratedCDCService(
        DataStorageService storage,
        JobArguments jobArguments
    ) {
        this.storage = storage;
        this.curatedTablePath = jobArguments.getCuratedS3Path();
    }
    ...
    public void applyUpdates(SparkSession spark, Dataset<Row> latestCDCRecordsByPK, SourceReference sourceReference) throws DataStorageRetriesExhaustedException {
        storage.mergeRecords(spark, curatedTablePath, latestCDCRecordsByPK, sourceReference.getPrimaryKey(), Arrays.asList(OPERATION, TIMESTAMP));
