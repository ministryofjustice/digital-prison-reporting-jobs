...
public class DomainServiceTest extends BaseSparkTest {
    ...
    public void shouldIncrementallyRefreshRecordsForCDCOperations(DMS_3_4_6.Operation operation) throws Exception {
        ...
        val tableRecord = createTableRow();
        ...
        when(mockJobArguments.getDomainRegistry()).thenReturn(domainTableName);
        when(mockDomainExecutor.getDomainDefinitions(spark, domainTableName))
                .thenReturn(Collections.singletonList(domainDefinition));

        when(mockDomainExecutor.applyTransform(any(), any()))
                .thenReturn(
        ...
        underTest.processIncrementally(spark, recordsToInsert, tableRecord);
    ...
    private DomainDefinition createDomainDefinition() {
        val sourceWithTableName = domainName + "_" + domainTableName;
        ...
        val sources = Collections.singletonList("system." + sourceWithTableName);

        transform.setSources(sources);
        transform.setViewText(
                "SELECT " +
                        "system." + sourceWithTableName + ".table_id as id, " +
                        "system." + sourceWithTableName + ".table_description as description " +
                        "from system." + sourceWithTableName
        );
