...
class CompactionJobTest extends SparkTestBase {
    ...
    @Test
    void shouldCompactConfiguredTables() {
    ...
    @Test
    void shouldCompactTablesInRecurseDepthOfRootPathWhenNoConfigIsProvided() {
        when(arguments.getMaintenanceTablesRootPath()).thenReturn(ROOT_PATH);
        when(arguments.getMaintenanceListTableRecurseMaxDepth()).thenReturn(RECURSE_MAX_DEPTH);
        ...
        verify(maintenanceService, times(1))
                .compactDeltaTables(spark, ROOT_PATH + "/", RECURSE_MAX_DEPTH);
    }
